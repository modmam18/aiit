...
if(eingabeString("\nName der Tour: ", nt, 100)){
  if(eingabeZahl("\nLänge in km: ", 1, 9999, &l)){
    if(eingabeZahl("\nAnzahl der Fahrten: ", 1, 9999, &f)){
      if(eingabeZahl("\nHöhenmeter der Strecke: ", 1, 9999, &hm)){
...
int merker = 0;
    ...
    tour = (struct radtour)realloc(tour, sizeof(struct radtour) * (*anzahl));
  return 0;
}
...
int main(){
   ...
   struct radtour *radtouren;

   radtouren = (struct radtour)malloc(sizeof(struct radtour));

  struct radtour radtouren2;

  radtouren2 = (struct radtour)malloc(sizeof(struct radtour));
...
//eingabeZahl("\n\nMenue-Auswahl: ", 1, 6, &a);
 ...
 case 3:
      ...
      ausgabeSchueler(radtouren, tourenanzahl);
      break;
      radtouren = (struct Radtour)realloc(radtouren, sizeof(struct radtour) * (tourenanzahl));
